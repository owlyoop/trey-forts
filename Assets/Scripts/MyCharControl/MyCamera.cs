	...
	public class MyCamera : MonoBehaviourPunCallbacks
	{
		...
		public void UpdateWithInput(float deltaTime, float zoomInput, Vector3 rotationInput)
		{
			if (FollowTransform)
			{
				...
				if (Physics.Raycast(Camera.transform.position, Camera.transform.forward, out shot, 2000f, layermask))
				{
					LookAtPosition = shot.point;
				}
				else
				{
					LookAtPosition = Camera.transform.position + (Camera.transform.forward * 2000f);
				}
			}
		}
	}
