    ...
    public class SmoothSyncEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (showMiscellaneous)
            {
                ...
                myTarget.isSmoothingAuthorityChanges = EditorGUILayout.Toggle(contentSmoothAuthorityChanges, myTarget.isSmoothingAuthorityChanges);
                GUIContent useVelocityForSyncing = new GUIContent("Use Velocity Driven Syncing", "Set velocity instead of position on non-owned objects.");
                myTarget.setVelocityInsteadOfPositionOnNonOwners = EditorGUILayout.Toggle(useVelocityForSyncing, myTarget.setVelocityInsteadOfPositionOnNonOwners);
                if (myTarget.setVelocityInsteadOfPositionOnNonOwners == true)
                {
                    GUIContent maxPositionDifference = new GUIContent("Max Position Difference", "If the difference between where it is and where it should be hits this, then it will go to location next frame. Is on an exponential scale otherwise. Only used for Velocity Driven Syncing.");
                    myTarget.maxPositionDifferenceForVelocitySyncing = EditorGUILayout.FloatField(maxPositionDifference, myTarget.maxPositionDifferenceForVelocitySyncing);
                }
            }
