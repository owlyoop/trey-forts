...
#region Configuration
        ...
        /// <remarks>
        /// The estimated owner time will shift by at most this amount (in seconds) when receiving an update. Lower values will 
        /// be smoother but it may take a bit to adjust to larger jumps in latency. Probably keep this lower than .05 unless you 
        /// are having serious latency issues. 
        /// </remarks>
        [Range(0, 5)]
        public float timeCorrectionSpeed = .01f;
        ...
        /// </remarks>
        public float snapTimeThreshold = 5.0f;
...
#region Internal stuff
        ...
        void interpolate(float interpolationTime)
        {
            ...
            {
                targetTempState.copyFromState(stateBuffer[1]);
                stopLerping();
                return;
            }
        ...
        public void teleport()
        {
            if (!hasAuthority)
            {
                Debug.LogWarning("Trying to teleport from an unowned object. You can only send RPCs from an owned object. Look up Unity networking object ownership.");
                return;
            }
            if (NetworkServer.active)
            {
        ...
        void addTeleportState(State teleportState)
        {
            ...
            if (teleportState.ownerTimestamp > stateBuffer[0].ownerTimestamp)
            {
                // Shift the buffer, deleting the oldest State.
...
#region Sync Properties
    ...
    bool isObservedByConnection(NetworkConnection conn)
        ...
        static void HandleSyncFromServerToNonOwners(NetworkMessage msg)
        {
            ...
            {
                networkState.smoothSync.adjustOwnerTime(networkState.state.ownerTimestamp, networkState.state);
...
#region Time stuff
        ...
        void adjustOwnerTime(float ownerTimestamp, State receivedState) 
        {
            ...
            if (Time.realtimeSinceStartup - firstReceivedMessageZeroTime < 1.0f || 
                timeChangeMagnitude < timeCorrectionSpeed || 
            ...
            else
            {
                ...
                {
                    approximateNetworkTimeOnOwner += timeCorrectionSpeed;
                }
                else
                {
                    approximateNetworkTimeOnOwner -= timeCorrectionSpeed;
